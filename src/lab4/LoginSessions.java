...
public class LoginSessions extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for (Cookie cookie : cookies) {
			for (Student student : students) {
				if (student.getCookie().equals(cookie.getValue())) {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (username == null || username.trim().length() == 0 || password == null || password.trim().length() == 0) {
			...
			// Don't forget, calling `doGet` does not stop the execution of this
			// method.
			// We need to `return`.
			return;
		}
		...
		// and comparing the submitted credentials against each student's email
		// and password
		ArrayList<Student> students = (ArrayList<Student>) getServletContext().getAttribute("students");
		...
		for (Student student : students) {
			if (student.getEmail().toLowerCase().equals(username.trim().toLowerCase())
				...
				// If we get here, the username and password match the current
				// `student`.
				// Let's create a session attribute that references `this
				// current student`.
				HttpSession session = request.getSession();
				session.setAttribute("authenticatedStudent", student);
				...
				try {
					if (request.getParameter("rememberMe") != null && request.getParameter("rememberMe").equals("on")) {
						...
						// creating the hexString
						Integer id = (Integer) student.getId();
						MessageDigest digest;
						digest = MessageDigest.getInstance("SHA-256");
						byte[] encodedHash = digest.digest(Integer.toHexString(id).getBytes(StandardCharsets.UTF_8));
						...
						// adding that hex string to the new "student" cookie
						Cookie cookie = new Cookie("student", hexString.toString());
						student.setCookie(cookie.getValue());
		...
		// if we get here then we couldn't find a Student that matched the
		// submitted credentials
		// So, we add an error message to the request scope and redisplay the
		// form
